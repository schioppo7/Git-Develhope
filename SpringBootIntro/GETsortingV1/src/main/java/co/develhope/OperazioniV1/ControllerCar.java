...
public class ControllerCar {
    ...
    //GET SORTING E PAGING
    @GetMapping("/sorted")
    public ResponseEntity<Page<Car>> getAllPagedSorted() {

        Sort sort = Sort.by("type").descending().and(Sort.by("color").ascending().and(Sort.by("id").ascending()));

        Pageable page = PageRequest.of(0,10, sort);
        Page<Car> carPage = carRepository.findAll(page);
